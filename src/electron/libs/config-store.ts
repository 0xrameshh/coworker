...
export type Avatar = {
  type: AvatarType;
  value: string; // initials: auto, emoji: emoji char, image: base64 data URL
};
...
export function getThemeSettings(): { theme: Theme; themePreset: string; accentColor: string; nightMode: boolean } {
  const config = loadFullConfig();
  return {
    theme: config.api.theme ?? "system",
    themePreset: config.api.themePreset ?? "default",
    accentColor: config.api.accentColor ?? "orange",
    nightMode: config.api.nightMode ?? false
  };
}

export function saveThemeSettings(settings: { theme?: Theme; themePreset?: string; accentColor?: string; nightMode?: boolean }): void {
  const config = loadFullConfig();
  if (settings.theme !== undefined) config.api.theme = settings.theme;
  if (settings.themePreset !== undefined) config.api.themePreset = settings.themePreset;
  if (settings.accentColor !== undefined) config.api.accentColor = settings.accentColor;
  if (settings.nightMode !== undefined) config.api.nightMode = settings.nightMode;
  saveFullConfig(config);
}
