...
export const generateSessionTitle = async (userIntent: string | null) => {
  ...
  try {
    const result: SDKResultMessage = await unstable_v2_prompt(
      ...
      model: claudeCodeEnv.ANTHROPIC_MODEL || "claude-opus-4-5-20251101",
      env: getEnhancedEnv(),
    ...
    console.warn("[util] Title generation returned non-success:", result.subtype);
    return "New Session";
  } catch (error) {
    console.error("[util] Failed to generate session title:", error);
    // Return a fallback title based on the first few words of the prompt
